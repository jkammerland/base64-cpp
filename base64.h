...
std::string base64_encode(std::string_view input) {
  ...
  if (encoded_length != static_cast<int>(output_length)) {
    throw std::runtime_error("Encoding error: predicted " +
                             std::to_string(output_length) + " but got " +
...
std::array<unsigned char, 44>
base64_encode(const std::array<unsigned char, 32> &input) {
  ...
  auto encoded_length = EVP_EncodeBlock(output.data(), input.data(),
                                        static_cast<int>(input_length));
...
std::string base64_decode(std::string_view input) {
  ...
  if (decoded_length != static_cast<int>(output_length)) {
    throw std::runtime_error("Decoding error: predicted " +
                             std::to_string(output_length) + " but got " +
...
std::array<unsigned char, 33>
base64_decode(const std::array<unsigned char, 44> &input) {
  constexpr size_t input_length = 44;
  constexpr size_t output_length = 32 + 1; // ceil(44 / 4) * 3 = 33

  std::array<unsigned char, output_length> output;

  auto decoded_length = EVP_DecodeBlock(output.data(), input.data(),
                                        static_cast<int>(input_length));
